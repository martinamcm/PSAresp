...
server <- function(input, output) {
  ...
  output$ggwaterfall<-renderPlot({
    ...
    PSAscore<-DataInf()[,1]
    PSAdat <- data.frame(id=seq(1,length(PSAscore)) ,PSA=PSAscore[rev(order(PSAscore))])
  ...
  Analysis <- eventReactive(input$Run,{
    ...
    applyaugbin(DataInf()[,1], input$Thres)
    applyaugbin(DataInf()[,1], input$Thres)
  ...
  output$Results <- renderFormattable({
    ...
    PointEst <- Analysis()[c(4,1)]
    CIlower <- Analysis()[c(5,2)]
    CIupper <- Analysis()[c(6,3)]
